...
parser.add_argument('--column', '-c', default=3, type=int, help='Column to use as samples from the datafile when datafile contains more than one column. Starts counting at 0. Default is 3 (the k column in the files this program was originally written for).')
...
#first argument is the x-value to be evaluated, remaining arguments are height, x-position, and width of first, second, ... guassian
def multi_gauss(x, *args):
  #number of gaussians to superimpose is inferred from the number of parameters given
  n = len(args)/3
  y = 0.0
  for i in range(n):
